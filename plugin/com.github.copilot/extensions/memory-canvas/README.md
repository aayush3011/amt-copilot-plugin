# Memory House canvas extension

A "Memory" panel for the GitHub Copilot app. Bundled inside the `memory-house` plugin
(Option 1). It shows what Memory House remembers about the signed-in developer, grouped into
...
**Customize -> Canvas** view; `/mh-show` opens it in the session's right side panel.
...
- **Reads** come straight from the Memory House REST API over the IP gateway, using the gateway-issued
  hook token from `mh-token.sh`. `whoami` resolves
  the caller; personal memories come from `GET /memories`, and team/org from scoped
  `POST /search`.
- **Writes** (`forget_memory`, `promote_memory`) do not touch Memory House directly. They call
  `session.send(...)` to ask the host agent to perform the action via the `memory-house`
...
- `/mh-login` completed once so the canvas can reuse the plugin's hook token.
...
- **Auth**: the canvas, automatic hooks, and import workflow share the same locally cached
  gateway hook token. `/mh-login` creates it and `mh-token.sh` refreshes it silently.
...
- **Session loading**: after installing or updating the plugin, start a new session so the
  app launches the updated canvas extension and adds `memory-house` to that session's canvas
  registry.
- **forget / promote**: there is no Memory House delete or promotion REST endpoint yet, so these
  actions delegate to the agent + MCP tools rather than calling Memory House directly. A first-class
