...
reports "not signed in", run `/mh-login` first, then retry.
...
```bash
SCRIPTS="$(ls -d "${COPILOT_HOME:-$HOME/.copilot}"/installed-plugins/*/memory-house/com.github.copilot/scripts | head -1)"
```

Windows (PowerShell):

```powershell
$SCRIPTS = (Get-ChildItem -Path (Join-Path $HOME '.copilot/installed-plugins') -Recurse -Directory -Filter scripts | Where-Object { $_.FullName -match 'memory-house' } | Select-Object -First 1).FullName
```
...
```
node "$SCRIPTS/mh-import.mjs" import-memories
node "$SCRIPTS/mh-import.mjs" import-memories --source claude
```
...
1. List the available sessions. Each entry shows an id, a turn count, a derived label, and a
   ...
   ```
   node "$SCRIPTS/mh-import.mjs" list-sessions
   node "$SCRIPTS/mh-import.mjs" list-sessions --source claude
   ```
2. Show the user that list and ask which sessions to import. Then import the chosen ids
   ...
   ```
   node "$SCRIPTS/mh-import.mjs" import-sessions <session_id> [<session_id> ...]
   node "$SCRIPTS/mh-import.mjs" import-sessions --all --source claude
   ```
...
available as a button in the Memory House canvas.
