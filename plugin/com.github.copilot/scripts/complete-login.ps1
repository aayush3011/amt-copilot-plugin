...
try { $payload = $raw | ConvertFrom-Json -ErrorAction Stop } catch {
  ...
  Write-SafeResult 'Memory House sign-in could not read the enrollment result. Call enroll_hook_capture once more. Never display enrollment credentials.'
  exit 0
}
...
if (-not $code) {
  ...
  Write-SafeResult 'Memory House sign-in could not extract the enrollment credential. Call enroll_hook_capture once more. Never display enrollment credentials.'
  exit 0
}
...
try {
  & pwsh -NoProfile -File (Join-Path $PSScriptRoot 'mh-login.ps1') *> $null
...
if ($loginExitCode -eq 0) {
  ...
  Write-SafeResult 'Memory House sign-in completed locally. The enrollment credential was redeemed and must not be displayed. Tell the user: Signed in to Memory House. Capture and recall are now active on this device.'
} else {
  Write-AmtHookLog 'login:auto:failed:redeem'
  Write-SafeResult 'Memory House sign-in was not completed because the enrollment credential was invalid or expired. Call enroll_hook_capture one more time; the automatic login hook will redeem the new credential. Never display enrollment credentials.'
}
