...
node (Join-Path $SCRIPTS 'mh-import.mjs') import-memories --source claude
```
...
If any step reports that the device is not signed in, run `/mh-login` first, then retry once.
