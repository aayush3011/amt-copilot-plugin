...
# 1) capture the user turn (fire-and-forget; never block the prompt).
try {
  Invoke-RestMethod -Method Post -Uri "$script:AmtHookBase/capture" -Headers $headers `
    -ContentType 'application/json' -Body (@{ thread_id = $thread; role = 'user'; content = $prompt } | ConvertTo-Json) -TimeoutSec 12 | Out-Null
} catch { }
